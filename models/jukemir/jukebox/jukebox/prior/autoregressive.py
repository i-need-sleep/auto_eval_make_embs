...
class ConditionalAutoregressive2D(nn.Module):
    ...
    def forward(self, x, x_cond=None, y_cond=None, encoder_kv=None, fp16=False, loss_full=False,
                encode=False, get_preds=False, get_acts=False, get_sep_loss=False, return_hidden_states=[]):
        # Preprocess.
        with t.no_grad():
            x = self.preprocess(x)
        ...
        x = self.transformer(x, encoder_kv=encoder_kv, fp16=fp16, return_hidden_states=return_hidden_states) # Transformer
        if len(return_hidden_states) > 0:
            x, hidden_states = x
