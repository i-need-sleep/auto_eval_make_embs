...
class Transformer(nn.Module):
    ...
    def forward(self, x, encoder_kv=None, sample=False, fp16=False, fp16_out=False, return_hidden_states=[]):
        if fp16:
            x = x.half()

        if len(return_hidden_states) > 0:
            # Make a list to store the intermediate hidden states
            hidden_states = []
